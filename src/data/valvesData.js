import { CATEGORIES } from "./presetsData";
...
const PRICES_STD_USD_RAW = {
  ...
  "FDV-R-HH0": { '1.5"': 1325, '2"': 1325, '3"': 1510 },
  "FDV-R-HHP": { '1.5"': 1636, '2"': 1636, '3"': 1822 },
  "FDV-R-MH0": {
    '1.5"': 972,
    '2"': 972,
    '3"': 1123,
...
const PRICES_HG_USD_RAW = {
  ...
  "FDV-R-HH0": { '1.5"': 1697, '2"': 1697, '2 1/2 "': 1798, '3"': 1899 },
  "FDV-R-HHP": { '1.5"': 2163, '2"': 2163, '2 1/2 "': 2264, '3"': 2365 },
  "FDV-R-MH0": {
    '1.5"': 1439,
    '2"': 1439,
    '2 1/2 "': 1521,
...
// פונקציית נרמול חכמה שמטפלת במבנה המערך, ברווחים ובשמות המפתחות מהאקסל
const smartNormalizePrices = (inputData) => {
  if (!inputData) return {};

  // אם הנתונים מגיעים כמערך (כמו שרואים בקונסולה)
  if (Array.isArray(inputData)) {
    const normalized = {};
    inputData.forEach((row) => {
      // חילוץ הקוד תוך התעלמות מרווחים נסתרים או מרכאות בשם המפתח
      const codeKey = Object.keys(row).find(
        (k) => k.trim().replace(/["']/g, "") === "Code"
      );
      const code = codeKey ? row[codeKey]?.toString().trim() : null;
      ...
      Object.keys(row).forEach((rawKey) => {
        ...
        // התעלמות משדות מטא כגון Code, Actuation או שורות ריקות
        if (
          trimmedKey === "Code" ||
          trimmedKey === "Actuation" ||
          trimmedKey === "__rowNum__"
        ) {
          return;
        }
        ...
        // זיהוי המידה 2 1/2 בכל וריאציה אפשרית והמרתה למפתח האחיד 2.5"
        if (cleanKey === "2 1/2" || cleanKey.includes("2 1/2")) {
          if (rawKey.includes("HG")) {
            newRow['2.5"_HG'] = val;
          } else {
            newRow['2.5"'] = val;
          }
        } else {
          newRow[rawKey.trim()] = val;
        }
      });

      normalized[code] = newRow;
    });
    return normalized;
  }

  // גיבוי למקרה שמדובר באובייקט רגיל
  const normalized = {};
  Object.keys(inputData).forEach((code) => {
    const row = inputData[code];
    if (!row) return;
    const newRow = {};
    Object.keys(row).forEach((rawKey) => {
      const val = row[rawKey];
    ...
    normalized[code.trim()] = newRow;
  });

  return normalized;
};

export const PRICES_STD_USD = smartNormalizePrices(PRICES_STD_USD_RAW);
